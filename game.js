(() => {
  ...
  const morseThreeWords = [
    "AIR","ANT","ARM","BAG","BAT","BED","BEE","BIG","BOX","BOY","BUS","CAN","CAP","CAR","CAT","COW","CUP","DAY","DOG","EAR",
    "EAT","EGG","EYE","FAN","FAR","FIT","FOX","GET","HAT","HOT","ICE","INK","JAM","JET","KEY","LEG","LIP","MAP","NET","NEW",
    "OLD","ONE","PEN","PIG","RED","RUN","SEA","SIT","SKY","SUN","TEA","TEN","TOP","TOY","TWO","VAN","WET","WIN","YES","ZOO"
  ];

  const morseFourWords = [
    "BABY","BACK","BALL","BANK","BEAR","BIRD","BLUE","BOAT","BODY","BOOK","CAKE","CITY","COLD","COOK","DARK","DOOR","DOWN","DRAW","EAST","FACE",
    "FAST","FIRE","FISH","FOOD","FOOT","GAME","GIFT","GIRL","GOOD","HAND","HEAD","HIGH","HOME","JUMP","KIND","KING","LAKE","LEFT","LINE","LONG",
    "LOOK","LOVE","LUNCH","MAKE","MEAT","MILK","MOON","NAME","NEAR","OPEN","PARK","PLAY","RAIN","READ","RICE","ROAD","ROOM","SAFE","SCHOOL","SHIP",
    "SHOP","SLOW","SNOW","STAR","STOP","SWIM","TALL","TREE","WALK","WARM","WASH","WIND","WORD","WORK","YARD","YEAR"
  ].filter(word => word.length === 4);

  const morseSixWords = [
    "ANIMAL","ANSWER","BANANA","BASKET","BOTTLE","BRIDGE","BRIGHT","BUTTON","CAMERA","CHANGE","CHEESE","CHOOSE","CIRCLE","CLOTHES","COFFEE","COLOUR","DANGER","DINNER","DOCTOR","DRIVER",
    "ENERGY","FAMILY","FINGER","FLOWER","FOREST","FRIEND","GARDEN","GROUND","GUITAR","HAMMER","HAPPY" ,"ISLAND","JACKET","JUNGLE","KITCHEN","LITTLE","MARKET","MIDDLE","MORNING","MOTHER",
    "MUSEUM","NUMBER","ORANGE","PEOPLE","PENCIL","PURPLE","RABBIT","REALLY","RIVER" ,"SCHOOL","SHOULD","SILVER","SIMPLE","SPRING","STREET","SUMMER","TABLET","TEACHER","TICKET",
    "TOMATO","TRAVEL","TURTLE","WINDOW","WINTER","YELLOW"
  ].filter(word => word.length === 6);

  function morseWordPool(length) {
    if (length === 3) return morseThreeWords;
    if (length === 4) return morseFourWords;
    if (length === 5) return scrollWordBank;
    return morseSixWords;
  }

  function morseLengthsForDifficulty(difficulty) {
    if (difficulty === "easy") return [3, 3, 3];
    if (difficulty === "medium") return [3, 4, 4];
    if (difficulty === "hard") return [4, 5, 5];
    return [5, 6, 6];
  }

  function encodeMorseWord(word) {
    return word.split("").map(letter => morseCodeMap[letter]);
  }

  function chooseMorseWord(length, alreadyUsed = []) {
    const pool = morseWordPool(length);
    const recent = campaign?.recentMorseWords || [];
    let candidates = pool.filter(word => !recent.includes(word) && !alreadyUsed.includes(word));
    if (!candidates.length) candidates = pool.filter(word => !alreadyUsed.includes(word));
    if (!candidates.length) candidates = pool;
    return choice(candidates);
  }

  function makeMorsePuzzle(difficulty) {
    const lengths = morseLengthsForDifficulty(difficulty);
    const targets = [];
    for (const length of lengths) {
      targets.push(chooseMorseWord(length, targets));
    }

    if (campaign) {
      campaign.recentMorseWords = campaign.recentMorseWords || [];
      targets.forEach(word => campaign.recentMorseWords.push(word));
      while (campaign.recentMorseWords.length > 24) campaign.recentMorseWords.shift();
    }

    return {
      targets,
      codes: targets.map(encodeMorseWord),
      entries: ["", "", ""],
    };
  }
  ...
  function makeScrollPuzzle(difficulty) {
    ...
    return { word, codes, currentLetters };
  }
  ...
  function renderMorse(module) {
    addInstruction("Tell your partner the three printed Morse lines. Type each decoded word, then press ENTER.");

    const wrap = document.createElement("div");
    wrap.className = "morse-lines";
    const inputs = [];

    module.codes.forEach((groups, index) => {
      const row = document.createElement("div");
      ...
      input.maxLength = module.targets[index].length;
      input.autocomplete = "off";
      input.autocapitalize = "characters";
      input.spellcheck = false;
      input.setAttribute("aria-label", `Morse line ${index + 1} answer`);
      input.value = module.entries[index] || "";

      input.addEventListener("input", () => {
        const cleaned = input.value.toUpperCase().replace(/[^A-Z]/g, "").slice(0, input.maxLength);
        input.value = cleaned;
      ...
      groups.forEach(group => {
        ...
        part.setAttribute("aria-label", group.split("").map(mark => mark === "." ? "dot" : "dash").join(" "));

        group.split("").forEach(mark => {
    ...
    const submitMorse = () => {
      ...
      strike("One or more Morse answers are wrong.");
      module.entries = ["", "", ""];
