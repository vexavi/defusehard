(() => {
  ...
  let strikeFlashOverlayEl = document.getElementById("strikeFlashOverlay");
  ...
  const teacherTimerMinutesEl = document.getElementById("teacherTimerMinutes");
  const teacherTimerSecondsEl = document.getElementById("teacherTimerSeconds");
  ...
  let warningAudioSchedulerId = null;
  let nextWarningBeepAt = 0;
  let continuousWarningOscillator = null;
  let continuousWarningGain = null;
  ...
  function showScreen(name) {
    ...
    document.body.classList.toggle("gameplay-active", name === "game");
  ...
  function clampTeacherTimeInput(input, min, max) {
    if (!input) return min;
  ...
  function teacherStartingSeconds() {
    const minutes = clampTeacherTimeInput(teacherTimerMinutesEl, 0, 99);
    const seconds = clampTeacherTimeInput(teacherTimerSecondsEl, 0, 59);
    return Math.max(1, minutes * 60 + seconds);
  }

  function updateTeacherTimerControls() {
    const disabled = !!teacherNoTimerEl?.checked;
    if (teacherTimerMinutesEl) teacherTimerMinutesEl.disabled = disabled;
    if (teacherTimerSecondsEl) teacherTimerSecondsEl.disabled = disabled;
  }
  ...
  function teacherSettings() {
    return {
      ...
      startingSeconds: teacherStartingSeconds(),
    };
  }
  ...
  function beginTeacherState({ levelNumber, config, modules, label }) {
    ...
    const startingSeconds = settings.noTimer ? config.seconds : settings.startingSeconds;
    state = {
      levelNumber,
      levelConfig: config,
  ...
  function getWarningAudioContext() {
    if (warningAudioContext) return warningAudioContext;
  ...
  function stopContinuousWarningTone() {
    if (continuousWarningGain && warningAudioContext) {
      const now = warningAudioContext.currentTime;
      try {
        continuousWarningGain.gain.cancelScheduledValues(now);
        continuousWarningGain.gain.setValueAtTime(
          Math.max(0.0001, continuousWarningGain.gain.value || 0.0001),
          now
        );
        continuousWarningGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.035);
      } catch (error) {}
    }

    if (continuousWarningOscillator) {
      try {
        continuousWarningOscillator.stop(
          warningAudioContext ? warningAudioContext.currentTime + 0.045 : 0
        );
      } catch (error) {}
    }

    continuousWarningOscillator = null;
    continuousWarningGain = null;
  }

  function startContinuousWarningTone() {
    if (continuousWarningOscillator) return;
    const context = getWarningAudioContext();
    if (!context || context.state !== "running") return;
    ...
    continuousWarningOscillator = oscillator;
    continuousWarningGain = gain;
  }

  function warningCadenceMs(secondsLeft) {
    if (secondsLeft <= 5) return 1000 / 12;
    if (secondsLeft <= 10) return 1000 / 6;
    if (secondsLeft <= 30) return 1000 / 3;
    if (secondsLeft <= 60) return 500;
    return 1000;
  }

  function stopWarningAudio() {
    if (warningAudioSchedulerId) {
      clearInterval(warningAudioSchedulerId);
      warningAudioSchedulerId = null;
    }
    nextWarningBeepAt = 0;
    stopContinuousWarningTone();
  }

  function startWarningAudio() {
    stopWarningAudio();
    if (!state || state.ended || state.teacherNoTimer) return;

    primeWarningAudio();
    nextWarningBeepAt = performance.now() + warningCadenceMs(state.timeLeft);

    warningAudioSchedulerId = setInterval(() => {
      if (!state || state.ended || state.teacherNoTimer || state.timeLeft <= 0) {
        stopWarningAudio();
        return;
      }

      // Final five seconds layer a continuous warning tone underneath 12 Hz beeps.
      if (state.timeLeft <= 5) {
        startContinuousWarningTone();
      } else if (continuousWarningOscillator) {
        stopContinuousWarningTone();
      }

      const now = performance.now();
      const cadence = warningCadenceMs(state.timeLeft);
      if (now >= nextWarningBeepAt) {
        playWarningBeep();
        nextWarningBeepAt = now + cadence;
      }
    }, 25);
  }
  ...
  // Keep this script at the end of index.html.
      ...
      if (state.timeLeft === 5) {
        nextWarningBeepAt = Number.POSITIVE_INFINITY;
        startContinuousWarningTone();
      }
  ...
  function triggerStrikeFlash(count) {
    const overlay = ensureStrikeFlashOverlay();
    if (!bombConsoleEl || !overlay) return;
    bombConsoleEl.classList.remove("strike-flash-once", "strike-flash-triple");
    overlay.classList.remove("active-once", "active-triple");
    void overlay.offsetWidth;

    if (count === 1) {
      bombConsoleEl.classList.add("strike-flash-once");
      overlay.classList.add("active-once");
      setTimeout(() => {
        bombConsoleEl.classList.remove("strike-flash-once");
        overlay.classList.remove("active-once");
      }, 520);
      return;
    }
    ...
    if (count >= 2) {
      ...
      overlay.classList.add("active-triple");
      setTimeout(() => {
        bombConsoleEl.classList.remove("strike-flash-triple");
        overlay.classList.remove("active-triple");
      }, 1180);
    }
  }
  ...
  function renderCurrentModule() {
    ...
    bombModuleEl.dataset.module = module.type;
    bombConsoleEl?.setAttribute("data-module", module.type);
  ...
  function renderWires(module) {
    ...
    module.wires.forEach((color, index) => {
      ...
      row.className = `wire wire-${color}`;
      row.style.setProperty("--wire-color", wireColor(color));

      const label = document.createElement("strong");
      label.className = "wire-label";
      label.innerHTML = `<span class="wire-index">${index + 1}</span><span class="wire-name">${color.toUpperCase()}</span>`;

      const assembly = document.createElement("div");
      assembly.className = "wire-line wire-assembly";
      assembly.innerHTML = `
        <span class="wire-terminal left" aria-hidden="true"></span>
        <span class="wire-cable-wrap" aria-hidden="true">
          <span class="wire-cable"></span>
          <span class="wire-shine"></span>
        </span>
        <span class="wire-terminal right" aria-hidden="true"></span>
      `;
      ...
      if (module.cut.includes(index)) row.classList.add("cut");
  ...
  teacherNoTimerEl?.addEventListener("change", updateTeacherTimerControls);
  teacherTimerMinutesEl?.addEventListener("change", () => clampTeacherTimeInput(teacherTimerMinutesEl, 0, 99));
  teacherTimerSecondsEl?.addEventListener("change", () => clampTeacherTimeInput(teacherTimerSecondsEl, 0, 59));
  updateTeacherTimerControls();
